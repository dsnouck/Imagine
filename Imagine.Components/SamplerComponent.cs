...
public class SamplerComponent(IColorComponent colorComponent, ILineComponent lineComponent) : ISamplerComponent
{
	...
	{
		var columnToX = ColumnToX(settings);
		var rowToY = RowToY(settings);

		return Enumerable.Range(0, settings.Rows)
			.AsParallel()
			.AsOrdered()
			.Select(row =>
				Enumerable.Range(0, settings.Columns)
					.Select(column =>
						colorComponent.Average(
	...
	private Func<double, double> ColumnToX(ImageSettings settings)
	{
		return lineComponent.LineThrough(
			new Vector2(-Half, settings.XMin),
			new Vector2((settings.Columns * settings.Subsamples) - Half, settings.XMax));
	}

	private Func<double, double> RowToY(ImageSettings settings)
	{
		return lineComponent.LineThrough(
